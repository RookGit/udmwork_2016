...
<div class="container-fluid">
    ...
    $exp_post = get_corrected_post($_POST['exp_post']);
    ...
    $graph_post = get_corrected_post($_POST['graph_post']);

    $gender_post = get_corrected_post($_POST['gender_post']);

    if ($_POST['age_post'] != null)
        $age_post = get_corrected_post($_POST['age_post']);
    else $age_post = 0;
        ...
        if ($_POST['my_result'] == $_POST['true_result']) {
            if ($_POST['header_post'] != null and $_POST['about_post'] != null and $_POST['number_post'] != null) {
                ...
                if (($result['rest'] - 1) > -1) {
                    ...
                    $query_black->setFetchMode(PDO::FETCH_ASSOC);
                    ...
                    if (isset($result_black['value'])) {
                        ...
                        $query = $db->prepare("UPDATE users SET rest = rest - 1, my_resume = my_resume + 1 WHERE id = :id;");
                        $query->execute($params);
                        ...
                        $query_2 = $db->prepare("INSERT INTO `resume` "
                            . "(`id`, `city`, `author`, `header`, `moder`, `payment`, `time`, `vip_time`, `see`, `type`, `graph_exp_gen_age`, `color`) VALUES "
                            . "(NULL, :city, :id, :header, (SELECT moder FROM users WHERE id=:id), :payment, :time, 0, 0, :type, :graph_exp_gen_age, :color);");
                        $params_2 = [
                            'id' => $_SESSION['id'],
                            'city' => $city_post,
                            'header' => $header_post,
                            ...
                            'time' => $time_plus,
                            'type' => $type_post,
                            'graph_exp_gen_age' => "$graph_post" . "$exp_post" . "$gender_post" . "$age_post",
                            'color' => "",
                        ];
                        ...
                        $query_3 = $db->prepare("SELECT id FROM resume WHERE author=:id and header = :header and time = :time");
                        $params_3 = [
                            'id' => $_SESSION['id'],
                            'header' => $header_post,
                            'time' => $time_plus,
                        ...
                        $query_4 = $db->prepare("INSERT INTO `resume_about` "
    ...
    if ($error == true) {
        $value_link = "plus_ads/resume";
        $style_window = "red";
        $value_button = "Назад";
    } else {
        $message = "Объявление успешно размещено.";
        $value_link = "my_cabinet/resume/1";
        $style_window = "yellow";
