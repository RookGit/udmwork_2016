...
<div class="container-fluid">
            ...
            if ($_POST['header_post'] != null and $_POST['about_post'] != null and $_POST['number_post'] != null) {
                ...
                if (($result['rest'] - 1) > -1) {
                    ...
                    if (isset($result_black['value'])) {
                        ...
                        $query = $db->prepare("UPDATE users SET rest = rest - 1, my_jobs = my_jobs + 1 WHERE id = :id;");
                        $query->execute($params);
                        ...
                        $query_2 = $db->prepare("INSERT INTO `udmwork`.`jobs` "
                            . "(`id`, `city`, `exp`, `author`, `header`, `moder`, `payment`, `time`, `vip_time`, `see`, `type`, `graph_gender`, `color`) VALUES "
                            . "(NULL, :city, :exp, :id, :header, (SELECT moder FROM users WHERE id=:id), :payment, :time, 0, 0, :type, :graph_gender, :color);");
                        $params_2 = [
                            'id' => $_SESSION['id'],
                            'city' => $city_post,
                            'exp' => $exp_post,
                            ...
                            'time' => $time_plus,
                            'type' => $type_post,
                            'graph_gender' => "$graph_post" . "$gender_post",
                            'color' => "",
                        ];
                        ...
                        $query_3 = $db->prepare("SELECT id FROM jobs WHERE author=:id and header = :header and time = :time");
                        $params_3 = [
                            'id' => $_SESSION['id'],
                            'header' => $header_post,
                            'time' => $time_plus,
                        ...
                        $query_4 = $db->prepare("INSERT INTO `udmwork`.`jobs_about` "
    ...
    if ($error == true) {
        $value_link = "plus_ads/jobs";
        $style_window = "red";
        $value_button = "Назад";
    } else {
        $message = "Объявление успешно размещено.";
        $value_link = "my_cabinet/jobs/1";
        $style_window = "yellow";
